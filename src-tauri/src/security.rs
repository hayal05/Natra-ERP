...
pub fn check_login(path: &std::path::Path, username: &str) -> Result<(), String> {
    ...
    let until: i64 = c.query_row("SELECT locked_until FROM login_security WHERE lower(username)=lower(?)", [username], |r| r.get(0)).optional().map_err(|e| e.to_string())?.unwrap_or(0);
    let now = now();
    if until > now {
        let remaining = until - now;
        return Err(format!("Account temporarily locked. Try again in {} minute(s).", (remaining + 59) / 60));
    }
    if until != 0 {
...
pub fn record_failure(path: &std::path::Path, username: &str) -> Result<(), String> {
    let c = Connection::open(path).map_err(|e| e.to_string())?;
    ensure_table(&c).map_err(|e| e.to_string())?;
    let current: i64 = c.query_row("SELECT failed_attempts FROM login_security WHERE lower(username)=lower(?)", [username], |r| r.get(0)).optional().map_err(|e| e.to_string())?.unwrap_or(0);
    let attempts = current + 1;
    let locked_until = if attempts >= MAX_FAILURES { now() + LOCK_SECONDS } else { 0 };
...
pub fn record_success(path: &std::path::Path, username: &str) -> Result<(), String> {
    let c = Connection::open(path).map_err(|e| e.to_string())?;
    ensure_table(&c).map_err(|e| e.to_string())?;
    c.execute("DELETE FROM login_security WHERE lower(username)=lower(?)", [username]).map_err(|e| e.to_string())?;
    Ok(())
}
