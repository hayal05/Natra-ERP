...
fn set_session(session: AuthSession) -> Result<(), String> {
    *session_store().lock().map_err(|_| "Authentication session is unavailable.".to_string())? = Some(session);
    Ok(())
}

fn clear_session() -> Result<(), String> {
    *session_store().lock().map_err(|_| "Authentication session is unavailable.".to_string())? = None;
    Ok(())
}

fn require_auth() -> Result<AuthSession, String> {
    session_store().lock().map_err(|_| "Authentication session is unavailable.".to_string())?.clone().ok_or_else(|| "Authentication required.".to_string())
}

fn require_hr_admin() -> Result<AuthSession, String> {
    let session = require_auth()?;
    if session.role != "hr_admin" {
        return Err("HR Admin authorization required.".to_string());
    }
    Ok(session)
}
...
#[tauri::command] pub fn employees_list(db:State<'_,Database>)->Result<Vec<Employee>,String>{require_auth()?;db.list_employees().map_err(|e|e.to_string())}
...
#[tauri::command] pub fn user_reset_password(db:State<'_,Database>,request:ResetPasswordRequest)->Result<(),String>{require_hr_admin()?;let password=request.new_password.trim();validate_password(password)?;let hash=hash_password(password)?;db.reset_password(&request.id,&hash,&now()).map_err(|_|"Unable to reset password for this user.".to_string())}
...
#[tauri::command] pub fn leave_list(db:State<'_,Database>,status:Option<String>)->Result<Vec<crate::leave::LeaveRequest>,String>{require_auth()?;crate::leave::list(&rusqlite::Connection::open(db.path()).map_err(|e|e.to_string())?,status.as_deref()).map_err(|e|e.to_string())}
...
#[tauri::command] pub fn turso_status()->Result<crate::turso::TursoConfig,String>{require_auth()?;crate::turso::status()}
...
#[tauri::command] pub async fn turso_test_connection()->Result<String,String>{require_hr_admin()?;let(url,token)=crate::turso::credentials()?;if url.is_empty()||token.is_empty(){return Err("Turso is not configured".into())}let remote=libsql::Builder::new_remote(url,token).build().await.map_err(|e|e.to_string())?;let conn=remote.connect().map_err(|e|e.to_string())?;conn.execute("SELECT 1",()).await.map_err(|e|e.to_string())?;Ok("connected".into())}
...
#[tauri::command] pub async fn sync_now(db:State<'_,Database>)->Result<crate::sync::SyncStatus,String>{require_auth()?;let(url,token)=crate::turso::credentials()?;if url.is_empty()||token.is_empty(){return Err("Turso is not configured".into())}crate::sync::sync_once(&db.path(),url,token).await}
