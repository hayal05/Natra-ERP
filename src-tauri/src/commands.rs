...
#[tauri::command] pub fn payroll_period(db:State<'_,Database>,period:String)->Result<crate::payroll::PayrollPeriod,String>{let conn=rusqlite::Connection::open(db.path()).map_err(|e|e.to_string())?;crate::payroll::period(&conn,&period).map_err(|e|e.to_string())}
