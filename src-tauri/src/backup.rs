...
fn is_natra_database(path:&Path)->Result<bool,String>{
    let c=Connection::open(path).map_err(|e|e.to_string())?;
    let has_users:i64=c.query_row("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='users'",[],|r|r.get(0)).map_err(|e|e.to_string())?;
    let has_employees:i64=c.query_row("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='employees'",[],|r|r.get(0)).map_err(|e|e.to_string())?;
    Ok(has_users>0 && has_employees>0)
}
...
pub fn create(path:&PathBuf, backup_dir:&Path)->Result<BackupInfo,String>{
    if !path.exists(){return Err("Database file does not exist.".into());}
    if !is_natra_database(path)?{return Err("The current database is not a valid NATRA ERP database.".into());}
    ...
    if !is_natra_database(&target)? || integrity(&target.to_path_buf())? != "ok" { let _=fs::remove_file(&target); return Err("Created backup failed validation.".into()); }
...
pub fn list(backup_dir:&Path)->Result<Vec<BackupInfo>,String>{
    ...
    for entry in fs::read_dir(backup_dir).map_err(|e|e.to_string())?{ let p=entry.map_err(|e|e.to_string())?.path(); if p.extension().and_then(|x|x.to_str())!=Some("sqlite3") || !p.file_name().and_then(|x|x.to_str()).map(|x|x.starts_with("natra-erp-")).unwrap_or(false){continue;} let meta=fs::metadata(&p).map_err(|e|e.to_string())?; items.push(BackupInfo{path:p.to_string_lossy().into_owned(),size_bytes:meta.len(),created_at:meta.modified().ok().and_then(|t|t.duration_since(UNIX_EPOCH).ok()).map(|d|d.as_millis().to_string()).unwrap_or_default()}); }
...
pub fn restore(path:&PathBuf, backup_path:&Path)->Result<BackupInfo,String>{
    ...
    fs::create_dir_all(&backup_dir).map_err(|e|e.to_string())?;
    let canonical_dir=backup_dir.canonicalize().map_err(|e|e.to_string())?;
    let canonical_backup=backup_path.canonicalize().map_err(|e|e.to_string())?;
    if !canonical_backup.starts_with(&canonical_dir){return Err("For safety, restore is limited to NATRA ERP backups.".into());}
    if canonical_backup==path.canonicalize().unwrap_or_else(|_|path.clone()){return Err("The active database cannot be used as its own restore source.".into());}
    if !canonical_backup.file_name().and_then(|x|x.to_str()).map(|x|x.starts_with("natra-erp-")).unwrap_or(false){return Err("Invalid NATRA ERP backup filename.".into());}
    if !is_natra_database(&canonical_backup)? {return Err("Selected file is not a valid NATRA ERP backup.".into());}
    if integrity(&canonical_backup.to_path_buf())? != "ok" {return Err("Selected backup failed integrity check.".into());}
    let safety=create(path, &backup_dir)?;
    let temp=path.with_extension("restore.tmp.sqlite3");
    let _=fs::remove_file(&temp);
    fs::copy(&canonical_backup,&temp).map_err(|e|e.to_string())?;
    if !is_natra_database(&temp)? || integrity(&temp)? != "ok" {let _=fs::remove_file(&temp);return Err("Restored database failed validation.".into());}
