...
pub async fn sync_once(path: &PathBuf, url: String, token: String) -> Result<SyncStatus, String> {
    ...
    for (outbox_id, entity, entity_id, payload) in items {
        let result = match entity.as_str() {
            "attendance" => { let v: serde_json::Value=serde_json::from_str(&payload).map_err(|e|e.to_string())?; conn.execute("INSERT INTO attendance(id,employee_id,attendance_date,check_in_at,status,token_id,created_at) VALUES(?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET employee_id=excluded.employee_id,attendance_date=excluded.attendance_date,check_in_at=excluded.check_in_at,status=excluded.status,token_id=excluded.token_id", params![v["id"].as_str().unwrap_or(&entity_id),v["employee_id"].as_str().unwrap_or(""),v["attendance_date"].as_str().unwrap_or(""),v["check_in_at"].as_str().unwrap_or(""),v["status"].as_str().unwrap_or("present"),v["token_id"].as_str().unwrap_or(""),v["check_in_at"].as_str().unwrap_or("")]).await }
            "leave" => { let v: serde_json::Value=serde_json::from_str(&payload).map_err(|e|e.to_string())?; conn.execute("INSERT INTO leave_requests(id,employee_id,leave_type,start_date,end_date,reason,status,reviewed_by,reviewed_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET employee_id=excluded.employee_id,leave_type=excluded.leave_type,start_date=excluded.start_date,end_date=excluded.end_date,reason=excluded.reason,status=excluded.status,reviewed_by=excluded.reviewed_by,reviewed_at=excluded.reviewed_at,updated_at=excluded.updated_at", params![v["id"].as_str().unwrap_or(&entity_id),v["employee_id"].as_str().unwrap_or(""),v["leave_type"].as_str().unwrap_or(""),v["start_date"].as_str().unwrap_or(""),v["end_date"].as_str().unwrap_or(""),v["reason"].as_str(),v["status"].as_str().unwrap_or("pending"),v["reviewed_by"].as_str(),v["reviewed_at"].as_str(),"",""]).await }
            "salary" => { let v: serde_json::Value=serde_json::from_str(&payload).map_err(|e|e.to_string())?; conn.execute("INSERT INTO salary_records(id,employee_id,pay_period,base_salary,allowances,deductions,net_salary,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET employee_id=excluded.employee_id,pay_period=excluded.pay_period,base_salary=excluded.base_salary,allowances=excluded.allowances,deductions=excluded.deductions,net_salary=excluded.net_salary,status=excluded.status,updated_at=excluded.updated_at", params![v["id"].as_str().unwrap_or(&entity_id),v["employee_id"].as_str().unwrap_or(""),v["pay_period"].as_str().unwrap_or(""),v["base_salary"].as_f64().unwrap_or(0.0),v["allowances"].as_f64().unwrap_or(0.0),v["deductions"].as_f64().unwrap_or(0.0),v["net_salary"].as_f64().unwrap_or(0.0),v["status"].as_str().unwrap_or("draft"),"",""]).await }
            _ => Err(libsql::Error::InvalidParameterName(format!("Unsupported sync entity: {entity}")))
        };
